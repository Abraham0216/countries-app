...
/* <div className="App">
    {responseData?.map(country => (
      <div key={country.cca3} onClick={() => handleCountryClick(country)} >
        ...
        <p>Native Name: {selectedCountry.name.nativeName ? Object.values(selectedCountry.name.nativeName).map(nativeName => nativeName.common).join(', ') : 'NO INFO'} </p>
        <p>Region: {selectedCountry.region}</p> 
        <p>population: {selectedCountry.population}</p> 
        <p>Sub Region: {selectedCountry.subregion}</p>
        <p>Capital: {selectedCountry.capital}</p> 
        <p>Top Domain: {selectedCountry.tld}</p> 
        <p>Currencies: {selectedCountry.currencies ? Object.values(selectedCountry.currencies).map(currency => currency.name).join(', ') : 'NO INFO'} </p>
        <p>Languages: {selectedCountry.languages ? Object.values(selectedCountry.languages).join(', ') : 'NO INFO'}</p>
